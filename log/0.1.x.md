...
#### Critical: Photometry Unit Conversion (commit 4faa277)
**Problem**: Aperture photometry stored surface brightness (MJy/sr) instead of integrated flux density.

**Impact**: CSV lightcurve files and plots displayed physically incorrect units - surface brightness instead of flux.

**Solution**:
- `processing/photometry.py`: Store flux in microJansky (μJy) instead of MJy/sr
  - Added conversion chain: MJy/sr → (× pixel_solid_angle) → MJy → (× 10^6) → Jy → (× 10^6) → μJy
  - Updated `PhotometryResult` to store `flux_ujy` and `flux_error_ujy`
  - Enhanced logging to show full conversion: `flux=123.456±7.890 μJy (0.000123±0.000008 Jy)`
- `core/config.py`: Updated `PhotometryResult` field comments: `MJy/sr` → `microJansky (μJy)`
- `processing/lightcurve.py`: Updated CSV column descriptions to reflect μJy units
- `visualization/plots.py`: Changed y-axis labels from `Flux (MJy/sr)` to `Flux Density (μJy)`
- `processing/magnitudes.py`: Clarified module docstring (magnitude calculation uses Jansky)

**Why μJy**: Standard unit for faint astronomical sources (JWST, HST). Typical SPHEREx flux range: 10-1000 μJy.

#### Documentation Fix (commit 620ae3c)
- `example/quickstart_demo.ipynb`: Corrected flag bit descriptions in notebook documentation

#### Example Update (commit 6b34795)
- Updated example files to reflect new flux units
...
**Git Tag**: v0.1.1 (commit cbfe898)

### Changes

#### Cleanup and Documentation
- **Removed obsolete files** (commit dd4021c):
  - Deleted `IMPLEMENTATION_PLAN.md` (258 lines)
  - Deleted `function design.md` (38 lines)
  - Deleted `setup.py` (40 lines) - now using Poetry exclusively
  - Updated `.gitignore` to reflect new structure

- **README overhaul** (commit 1c84268):
  - Simplified installation instructions
  - Condensed feature descriptions (70 lines vs 119 lines)
  - Improved clarity and removed redundant sections

- **Example updates** (commit 4e6895e):
  - Updated demo data and quickstart notebook
  - Refreshed example outputs (`cloverleaf.json`, `combined_plot.png`, `query_summary.json`)
