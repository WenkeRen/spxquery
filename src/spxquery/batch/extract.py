...
def process_single_image(
    ...
    """
    # Include parent dir (band, e.g. D4) in filename to avoid collisions
    # between spectral channels sharing the same obs_id (D1↔D4, D2↔D5, D3↔D6).
    band_dir = image_path.parent.name  # e.g. "D4"
    output_csv = output_dir / f"{image_path.stem}_{band_dir}_photometry.csv"
    if skip_existing:
    ...
    try:
        ...
        if results:
            ...
            output_filename = output_csv  # reuse the band-qualified name from above
            try:
                pd.DataFrame(results).to_csv(output_filename, index=False)
                evict_file_pages(output_filename, sync=True)
